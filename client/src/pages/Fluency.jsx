...
import { generateFluencyExercise, evaluateFluency } from '../services/fluencyService';
import { useSpeechRecognition } from '../hooks/useSpeechRecognition';
import { useTextToSpeech } from '../hooks/useTextToSpeech';
import LoadingSpinner from '../components/ui/LoadingSpinner';
import ErrorAlert from '../components/ui/ErrorAlert';
import ScoreRing from '../components/ui/ScoreRing';
import { HiOutlineSpeakerWave, HiOutlineMicrophone, HiOutlineSparkles, HiOutlinePencilSquare } from 'react-icons/hi2';
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-dark-400">Combine sentences, expand ideas, and improve your writing flow</p>
        </div>
      </div>
      ...
      {!exercise && !loading && (
        <div className="glass-card p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Fluency Exercise</h2>
          <p className="text-dark-400 mb-6">Get a set of sentences to combine, expand, remove repetitions, and create a story</p>
      ...
      {exercise && !result && (
        <div className="space-y-6">
          <div className="glass-card p-6">
            <h2 className="text-lg font-semibold text-white mb-4">📝 Your Exercise</h2>
            <div className="space-y-3">
              <div className="p-4 rounded-xl bg-primary-500/10 border border-primary-500/20">
                <p className="text-xs text-primary-400 font-semibold mb-1">STARTER</p>
                <p className="text-white">{exercise.starter}</p>
              </div>
              {exercise.sentences?.map((s, i) => (
                <div key={i} className="p-4 rounded-xl bg-dark-800">
                  <p className="text-xs text-dark-400 font-semibold mb-1">SENTENCE {i + 1}</p>
                  <p className="text-dark-200">{s}</p>
                </div>
              ))}
              <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                <p className="text-xs text-amber-400 font-semibold mb-1">INSTRUCTION</p>
                <p className="text-amber-200 text-sm">{exercise.instruction}</p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {transcript && (
                  <button onClick={handleUseVoiceInput} className="text-xs text-primary-400 hover:text-primary-300">
                    Use voice input
                  </button>
                )}
              </div>
            ...
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={handleGenerate} className="btn-secondary text-sm">New Exercise</button>
      ...
      {result && (
        <div className="space-y-6 animate-slide-up">
          <div className="glass-card p-6 text-center">
            <h2 className="text-xl font-bold text-white mb-6">Fluency Evaluation 🎯</h2>
            <div className="flex justify-center">
              <ScoreRing score={result.score || 0} size={100} label="Fluency Score" />
            </div>
          </div>
          ...
          {result.improved_version && (
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => speak(result.improved_version)} disabled={isSpeaking} className="btn-ghost text-sm flex items-center gap-1">
                  <HiOutlineSpeakerWave className="w-4 h-4" /> 🔊 Listen
                </button>
              </div>
          ...
          <button onClick={() => { setResult(null); setExercise(null); setUserAnswer(''); }} className="btn-primary">
